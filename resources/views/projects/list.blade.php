                ...
                <div class="card-body" style="height: 75vh;overflow: auto;">
                    ...
                    <ul class="list-group">
                        @foreach($projects as $project)
                            <li class="list-group-item d-flex justify-content-between align-items-center project{{ $project->id }}" onclick='showSubProjectPanel({{ $project->id }})'>
                                <div class="row">
                                    <div class="col-12 col-md-4 mb-2 mb-md-0" style="max-width:200px;">
                                        <img class="w-100" src="{{ asset($project->images->path) }}" alt="">
                                    </div>
                                    <div class="col-12 col-md-8">
                                        {{ $project->th_name }} ({{ $project->en_name }}) <br>
                                        
                                        <p class="pt-3">{!! str_limit($project->th_description, 200, '...') !!}</p>
                                        <p class="pb-2 mb-0">{!! str_limit($project->en_description, 200, '...') !!}</p>
                                    </div>
                                    <div class="col-12 text-right">
                                        <div class="d-inline-flex">
                                            <a href="{{route('projects.edit',[ 'id' => $project->id ])}}" class="btn btn-outline-secondary btn-sm mr-1">Edit</a>
                                            <!-- <form class="form-horizontal" action="{{route('projects.delete',[ 'id' => $project->id ])}}" method="POST">
                                                @csrf
                                                {{method_field('DELETE')}}
